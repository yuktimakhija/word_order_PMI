...
k_clusters = 128
def pmi_allwords_calc(clusters_pred, k_clusters=k_clusters):
	clusters = []
	...
	for corpus in corpora:
		...
		for file in collective:
			...
			# f = gen_feature_encoding.FeatureExtractor(train_sents, train_megam=False)
			...
			# 			total += w[f_id]*f_val
			# 		word_embs[i][j] = total
			# # kmeans_model = KMeans(n_clusters=n_clusters, random_state=42).fit(word_embs)
			# clusters_pred = kmeans_model.predict(word_embs)
			# print(clusters_pred.shape)
			# print('Silhouette score of clustering: ', silhouette_score(word_embs, clusters_pred, random_state=42))
			j = 0
			for line in lines:
				# print(line)
				if line[0].isnumeric():
				...
				# print(line[0])
				if word_count<=20  and not (line[0].isnumeric()):
					# print ('if worked!')
					# print('---------------', line)
					sentences_lesser_20words +=1 
					k = 1
					for i in cluster_sentence:
						for ii in cluster_sentence[k:]:
							# if tag_sentence[i]!='PUNCT' and tag_sentence[ii]!='PUNCT':
							dep_id = i
							# head_id = cluster_id[ii]
							head_id = ii
							count_arr[dep_id][head_id] += 1
							total_dep += 1
						k += 1
							# count_arr[head_id][dep_id] += 1
							# total_dep += 1
					cluster_sentence = []
		tdep += total_dep
		...
		np.savetxt(f"{k_clusters}/pmi_root_allwords_{corpus}.csv", pmi_matrix, delimiter=',', header=','.join(clusters), fmt='%.4f')
				# print(pmi_matrix[(i,j)])
		# print(count_arr)
		return pmi_matrix
		print(pmi_matrix)
	...
	# print(col_sum)
	# print(row_sum)
	# np.savetxt("PMI_root_allwords/pmi_all_datasets_combined.csv", pmi_matrix, delimiter=',', header=','.join(ud_tags))
