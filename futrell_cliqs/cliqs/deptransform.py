...
def collapse_flat(sentence, rels=FLAT_RELS, verbose=VERBOSE):
    ...
    for h in nodes:
        ...
        if flats and sequence_continuous(flats):
            ...
            new_word = " ".join(sentence.nodes[d]['word'] for d in words)
            sentence.nodes[h]['word'] = new_word
            ...
            for d in flats:
                for _, dd, dt in sentence.out_edges(d, data=True):
                    if dt['deptype'] not in rels:
                        sentence.add_edge(h, dd, attr_dict= dt)
...
def test_collapse_flat():
    ...
    assert len(s2) == 1
    assert s2.nodes[0]['word'] == "a b c d e"
    ...
    assert len(s2) == 1
    assert s2.nodes[0]['word'] == "a b c d"
...
def remove_from_sentence(sentence, badrel, badpos, verbose=VERBOSE, strict=True): 
    ...
    for punct, word2, deptype in punctuation_as_head:
        ...
        if word1 == 0:
            ...
            while (sentence.nodes[word1].get('pos') in badpos
...
def test_remove_punct_from_sentence():
    ...
    assert s2.nodes[1]['pos'] == 'NN'
    assert s2.nodes[2]['pos'] == 'VV'
    assert s2.nodes[3]['pos'] == 'PRP'
    assert s2.nodes[4]['pos'] == 'HELLO'
...
def test_reverse_content_head_simple():
    ...
    s.nodes[1]['pos'] = 'ADP' # on
    s.nodes[2]['pos'] = 'NOUN'  # Sunday
    s.nodes[3]['pos'] = 'PRON' # I
    s.nodes[4]['pos'] = 'VERB' # was
    s.nodes[5]['pos'] = 'NOUN'  # man
    s.nodes[6]['pos'] = 'SCONJ' # that
    s.nodes[7]['pos'] = 'VERB' # died
