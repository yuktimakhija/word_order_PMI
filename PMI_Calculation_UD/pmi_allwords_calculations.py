...
for corpus in corpora:
	...
	for file in collective:
		...
		for line in lines:
			...
			if word_count<=20  and not (line[0].isnumeric()):
				...
				k = 1
				for i in tag_sentence:
					for j in tag_sentence[k:]: 
						if j != 'PUNCT' and i!='PUNCT':
							dep_id = tag_id[i]
							head_id = tag_id[j]
	...
	np.savetxt(f"PMI_root_allwords/pmi_{corpus}.csv", pmi_matrix, delimiter=',', header=','.join(ud_tags))
			# print(pmi_matrix[(i,j)])
	# print(count_arr)
	print(pmi_matrix)
	# print(count_arr[(1,2)])
